...
contract VisualKey {
    ...
    constructor(
        ...
        address firstTokensReceiver,
        uint256 lockInterval,
        string memory tokenMetadataUri,
        uint256[] memory firstTokens
    ) {
        ...
        if (firstTokensReceiver == address(0)) {
        ...
        //#region Mint The First Tokens
        for (uint256 i = 0; i < firstTokens.length;) {
            uint256 token = firstTokens[i];

            _requireNotMinted(token);
            _mint(firstTokensReceiver, token);
